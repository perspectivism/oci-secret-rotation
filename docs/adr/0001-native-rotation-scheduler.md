...
**Harder:**
- `rotation_config` requires the Function OCID before scheduled rotation can be configured. The potential Terraform cycle (secret needs Function OCID; Function config needs secret OCID) is broken by removing the secret OCID from Function config entirely: the handler reads `secretId` from OCI's native four-step rotation payload at runtime instead. This makes the graph acyclic — Function first, vault secret second with `rotation_config` already wired — so a single `terraform apply` is sufficient for a fresh deployment.
